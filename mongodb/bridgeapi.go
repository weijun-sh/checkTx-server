...
func FindSwapResult(isSwapin bool, dbname, txid, pairID, bind string) (*MgoBridgeSwapResult, error) {
	...
	if isSwapin {
		...
		return findSwapResult(c, txid, pairID, bind)
	}
...
func FindSwap(isSwapin bool, dbname, txid, pairID, bind string) (*MgoBridgeSwap, error) {
	...
	if isSwapin {
		...
		database := client.Database(dbname)
		c := database.Collection(tablename)
		return findSwap(c, txid, pairID, bind)
	}
...
//func FindSwapin(txid, pairID, bind string) (*MgoBridgeSwap, error) {
...
//func FindSwapout(txid, pairID, bind string) (*MgoBridgeSwap, error) {
...
func findSwap(collection *mongo.Collection, txid, pairID, bind string) (*MgoBridgeSwap, error) {
...
func findSwapOrSwapResult(result interface{}, collection *mongo.Collection, txid, pairID, bind string) (err error) {
	fmt.Printf("findSwapOrSwapResult, txid: %v, pairID: %v, bind: %v\n", txid, pairID, bind)
	if bind != "" {
...
func findSwapResult(collection *mongo.Collection, txid, pairID, bind string) (*MgoBridgeSwapResult, error) {
	result := &MgoBridgeSwapResult{}
	err := findSwapOrSwapResult(result, collection, txid, pairID, bind)
	fmt.Printf("findSwapResult, result: %v, err: %v\n", result, err)
	if err != nil {
		return nil, err
	}
	return result, nil
