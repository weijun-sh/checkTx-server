...
// GetCheckBridgeTxHandler handler
func GetCheckBridgeTxhashHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bridge := vars["bridge"]
	txid := vars["txid"]
	ret := swapapi.GetFileLogs(bridge, txid, true)
	if ret == nil {
		writeResponse(w, nil, fmt.Errorf("%v '%v' not found", bridge, txid))
	} else {
		writeResponse(w, ret, nil)
	}
}
